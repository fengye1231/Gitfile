...
<div class="bjui-pageHeader">
    <form id="pagerForm" data-toggle="ajaxsearch" action="${ctx}/sys/query${actionclass1}.action" method="post">
        ...
        <div class="bjui-searchBar">
            <label>车次：</label><input type="text" value="" id="${actionclass1}_sid" name="s_sid" class="form-control" size="10">&nbsp;
<!--             <label>:</label> -->
    ...
    <table data-toggle="tablefixed" data-width="100%" data-nowrap="true">
        ...
        <tbody>
            ...
            <tr data-id="${item.id }">
               <td>${item.sid}</td>
               <td>${item.iccard}</td>
               <td>${item.startSta.name} - ${item.endSta.name}</td>
               <td>${item.linetime}</td>
               <td>${item.stations}</td>
               <td>${item.distance}
               <a href="${ctx}/sys/getLineDistance.action?uid=${item.id}" class="btn btn-green" data-toggle="navtab" data-options="{reloadWarn:''}" data-id="baseAdd" data-reload-warn="本页已有打开的内容，确定将刷新本页内容，是否继续？" data-title="更新站点距离">编辑</a>
               </td>
