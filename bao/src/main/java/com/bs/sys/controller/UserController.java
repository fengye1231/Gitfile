...
public class UserController {
    ...
    /*
    得到登陆用户下所有的学生，或者得到学生所在班级的老师
     */
    @RequestMapping("getattchedpersons")
    public List<User> getAttchedPersons(){
        List<User> list=new ArrayList<>();
        User user = (User) WebUtils.getSession().getAttribute("user");
        List<Integer> roleList=roleService.queryUserRoleIdsByUid(user.getId());
        String maxRolename="";
        for(Integer roleid:roleList){
            if(roleService.getById(roleid).getName().equals("老师")){
                maxRolename="老师";
            }
            if(roleService.getById(roleid).getName().equals("学生")){
                maxRolename="学生";
            }
        }
        if(maxRolename.equals("老师")){
            QueryWrapper<User> userQueryWrapper=new QueryWrapper<>();
            userQueryWrapper.eq("mgr",user.getId());
           list=userService.list(userQueryWrapper);
        }
        if(maxRolename.equals("学生")){
            QueryWrapper<User> userQueryWrapper=new QueryWrapper<>();
            userQueryWrapper.like("name",userService.getById(user.getMgr()).getName().substring(0,3));
            list=userService.list(userQueryWrapper);
        }
        return list;
    }
